...
public class ResourceExceptionHandler {
    ...
    public ResponseEntity<StandardError> handleResourceNotAvailableException(ResourceNotAvailableException e, HttpServletRequest request) {
		String error = "Product Not Available For Sale";
		HttpStatus status = HttpStatus.BAD_REQUEST;
		StandardError err = new StandardError(Instant.now(), status.value(), error, e.getMessage(),
				request.getRequestURI());
		return ResponseEntity.status(status).body(err);
    ...
    public ResponseEntity<StandardError> handleInvalidDataException(InvalidDataException e, HttpServletRequest request) {
